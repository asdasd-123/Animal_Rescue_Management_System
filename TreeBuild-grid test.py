...
class TreeBuild:
    ...
    * - Optional
    ...
    """
    def __init__(self, parent, data, headings, search=False, widths=[], grid=False, location=[]):
        """Will place a treeview with provided data set,
        complete with search boxes if needed.
        * - Optional
        parent  - parent frame
        ...
        search* - Default=False. True to add search boxes
        widths* - List of fixed column widths, otherwise auto selected
        grid*   - Is it housed in a grid or frame?
        location- If grid, location data as follows:
                  [drow,drowspan,dcol,dcolspan,hrow,hrowspan,hcol,hcolspan]
        """
        ...
        if len(self.location) == 0 and self.grid:
            self.location = [0, 0, 1, 1, 1, 0, 1, 1]
        ...
        # Build search area
        if self.search and self.grid:
            self._build_search_grid()
        elif self.search and not self.grid:
    ...
    def _build_search_grid(self):
        self.search_grid = ttk.Frame(self.parent, height="50")
        loc = self.location
